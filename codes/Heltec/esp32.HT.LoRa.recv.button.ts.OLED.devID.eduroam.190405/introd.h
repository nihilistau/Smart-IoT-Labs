...
int setAP()
{
int ap=0;  // ap=1 eduroam, ap=2 PhoneAP (smartcomputerlab)
int  buttonState;
u8x8.clear(); 
char dbuf[32];
u8x8.drawString(0, 1,"AP=1 no WiFi");
u8x8.drawString(0, 2,"AP=2 eduroam");
u8x8.drawString(0, 3,"AP=3 PhoneAP");
while(1)
  {  
  buttonState = digitalRead(buttonPin);
  if(buttonState) { ap++;if(ap==4) ap=1;
                    Serial.println(ap);
...
while(1)
  ...
  if(buttonState) 
    { 
      ...
      u8x8.clear();
      if(count) u8x8.draw2x2String(0, 3,"868 MHz");
      else u8x8.draw2x2String(0, 3,"433 MHz");
    }
  else
    {
    u8x8.clear();
    if(count) u8x8.drawString(0, 4,"Set to 868 MHz");
    else u8x8.drawString(0, 4,"Set to 433 MHz");
