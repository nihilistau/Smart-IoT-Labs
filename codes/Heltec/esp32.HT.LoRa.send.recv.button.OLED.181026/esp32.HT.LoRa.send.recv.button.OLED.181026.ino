...
void dispData(int sr, int rssi)  // sr=1 - send, sr=0 - receive
{
  char dbuf[32];
...
while(1)
  ...
  sdf.pack.head[1]= 0x03;  // source term 3
  ...
  sdf.pack.tsdata.sensor[3]=(float)urssi.rssi;
...
rssiqueue = xQueueCreate(4, 4);  // queue for rssi
...
void loop() {
 ...
 Serial.print("RSSI from queue:");Serial.println(urssi.rssi);
    dispData(0,urssi.rssi);
