    ...
    struct
      {
        int pnum;
        uint16_t macnum[2];
        uint8_t mac[20][6];  // 3 sensor values
      } pack;
  } sdf;  // data frame 
...
int sendmac()
{
  ...
  if(ti>0 && ti<21)
    {
    for(i=0;i<ti;i++) memcpy(sdf.pack.mac[i],tmac[i],6);

    LoRa.beginPacket();
    sdf.pack.pnum=count;
    sdf.pack.macnum[0]=1;
    sdf.pack.macnum[1]=(uint16_t)ti;
    LoRa.write(sdf.frame,128);
    LoRa.endPacket();
    }
    
  if(ti>20 && ti<41)
    {
    for(i=20;i<ti;i++) memcpy(sdf.pack.mac[i],tmac[i],6); 
    LoRa.beginPacket();
    sdf.pack.pnum=count;
    sdf.pack.macnum[0]=2;
    sdf.pack.macnum[1]=(uint16_t)ti;
  ...
  if(ti>40 && ti<61)
    {
    for(i=40;i<ti;i++) memcpy(sdf.pack.mac[i],tmac[i],6); 
    LoRa.beginPacket();
    sdf.pack.pnum=count;
    sdf.pack.macnum[0]=3;
    sdf.pack.macnum[1]=(uint16_t)ti;
...
while(1)
   ...
   if( xSemaphoreTake( xSemaphore, ( TickType_t ) 10 ) == pdTRUE )
        {
        sendmac();        Serial.println("LoRa packet sent"); 
        clearmac();
