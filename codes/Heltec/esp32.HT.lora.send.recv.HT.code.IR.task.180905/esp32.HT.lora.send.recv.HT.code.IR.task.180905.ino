...
int i=0, rssi, rdcount=0;

void dispData(int rs)  // received-0, send-1
{
char dbuf[32]; 
  u8x8.begin();
  ...
  u8x8.clear();
  memset(dbuf,0x00,32);
  if(rs) sprintf(dbuf,"SPacket:%4.4d",counter);
  else sprintf(dbuf,"RPacket:%4.4d",(int) rdf.pack.sensor[0]);
  u8x8.drawString(0, 1,dbuf);
  memset(dbuf,0x00,32);
  if(rs) sprintf(dbuf,"Temp:%2.2f",sdf.pack.sensor[2]);
  else sprintf(dbuf,"Temp:%2.2f",rdf.pack.sensor[2]);
  u8x8.drawString(0, 2,dbuf);
  memset(dbuf,0x00,32);
  if(rs) sprintf(dbuf,"Humi:%2.2f",sdf.pack.sensor[3]);
  else sprintf(dbuf,"Humi:%2.2f",rdf.pack.sensor[3]);
  u8x8.drawString(0, 3,dbuf);
  memset(dbuf,0x00,32);
  if(rs) sprintf(dbuf,"Code:%d",(int)sdf.pack.sensor[1]);
  else sprintf(dbuf,"Code:%d",(int)rdf.pack.sensor[1]);  // receiving RSSI
  u8x8.drawString(0, 4,dbuf);
  memset(dbuf,0x00,32);
  if(!rs) 
    { sprintf(dbuf,"RSSI:%d",(int)rssi);
...
void setup() {
  ...
  cqueue = xQueueCreate(4, 32); // queue for IR code
  ...
  xTaskCreate(
                    LoRaRecvTask,   /* Function to implement the task */
                    "coreTask", /* Name of the task */
                    10000,      /* Stack size in words */
                    NULL,       /* Task input parameter */
                    0,          /* Priority of the task */
  ...
  Serial.println("LoRaRecvTask created...");
...
char inbuff[32];
void loop() {
      Serial.println("waiting for item from IR");
