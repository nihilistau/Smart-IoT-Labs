...
#define EAP_ID "bakowski­p@univ­nantes.fr"
...
//rtc.adjust(DateTime(__DATE__, __TIME__));  // only for first run !
  ...
  api=setAP();
    ...
    if (!SD.begin(SD_CS, sd_spi))
      ...
      u8x8.clear();
      u8x8.drawString(0,2,"SD card mounted");
      u8x8.drawString(0,3,"data stored in");
      u8x8.drawString(0,4,"lora_recv.log");
      }
       ...
       if(constep==100) 
         {
          ...
          u8x8.clear();
          u8x8.drawString(0,2,"WiFi fails");
         }
       else 
         {
          apcon=1;
          ...
          delay(1000);
          u8x8.clear();
          u8x8.drawString(0,1,"WiFi OK");
          u8x8.drawString(0,3,"connected to");
          u8x8.drawString(0,4,"eduroam");
          u8x8.drawString(0,6,"ThingSpeak");
          u8x8.drawString(0,7,"client started");
...
void loop() {
  ...
  if (packetSize==24)
    ...
    dispData();   
    loralog = SD.open(LOG_PATH, FILE_APPEND);
        if (!loralog) 
          {
          ...
          memset(lora_buf,0x00,256);
          sprintf(lora_buf,"%2.2d/%2.2d/%2.2d - %2.2d:%2.2d:%2.2d - %2.2f,%2.2f,%2.2f,%d",year,month,day,hour,minute,second,rdf.pack.sensor[0],rdf.pack.sensor[1],rdf.pack.sensor[2],rssi);
          loralog.write((uint8_t *)lora_buf, strlen(lora_buf));
          loralog.printf("\n");
          loralog.close();
          }
