...
void onReceive(int packetSize) {
  ...
  if(rcf.pack.con[0]==0x02 && rcf.pack.did==sendID) // sent to this node 
    {
    timeout= rcf.pack.tout;     // received ACK-Data frame
    }
...
void send_Data(char *wkey, uint8_t mask, int chan, float *stab)  // write key , sensor mask, sensor table values
{
  ...
  sdf.pack.con[0]=0x01; sdf.pack.con[1]=mask;       // send data
  sdf.pack.channel= chan;
  strncpy(sdf.pack.keyword,wkey,16);
  for(int i=0;i<8;i++) { sdf.pack.sens[i]=stab[i]; Serial.print(sdf.pack.sens[i]);}
  sdf.pack.tout=0;
...
void loop() {
  if(ack==0) 
    ...
    send_Data(wkey, mask, chan, stab);   Serial.println("Data sent"); 
    LoRa.receive();  
    }
