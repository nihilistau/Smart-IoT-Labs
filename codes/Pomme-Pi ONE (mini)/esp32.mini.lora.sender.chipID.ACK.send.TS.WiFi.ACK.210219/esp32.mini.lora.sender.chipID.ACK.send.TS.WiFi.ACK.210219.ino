...
int read_TS()
{
  ...
  Serial.println(sdf.pack.sens[0]);
...
int send_Data_ACK(uint8_t idrd,uint16_t timeout)
{
  ...
  sdf.pack.con[0]=idrd; scf.pack.con[1]=0x00;       // ACK  - 0x01 for IDREQ ACK and 0x02 for DataACK
  sdf.pack.channel= rdf.pack.channel;
  sdf.pack.tout= timeout;
  LoRa.write(scf.frame,64);
  LoRa.endPacket();
}
...
if (packetSize==64)
  ...
  if(rdf.pack.con[0]==0x02 && rdf.pack.did==sendID)
    ...
    if (!client.connected()) { connect(); }
    read_TS(); 
    send_Data_ACK(0x02,tout);  // to be used or onot - e.g. timeout
    delay(200); 
    }
  }
packetSize=0;
