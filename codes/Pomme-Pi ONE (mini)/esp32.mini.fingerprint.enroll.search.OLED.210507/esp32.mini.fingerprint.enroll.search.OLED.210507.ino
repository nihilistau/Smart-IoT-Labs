...
{
    Serial.begin(9600);
    Serial.println("1:N MATCH test");disp("1:N MATCH test");
    fserial.begin(57600);
...
int search_database(void) {
    ...
    if (p == FPM_OK) {
        Serial.println("Found a print match!");disp("Found a print match!");
    } else if (p == FPM_PACKETRECIEVEERR) {
        Serial.println("Communication error");
        return p;
    ...
    sprintf(dbuff,"Found ID #%d\nScore=%d\n",fid,score);
    disp(dbuff);delay(4000);
}
