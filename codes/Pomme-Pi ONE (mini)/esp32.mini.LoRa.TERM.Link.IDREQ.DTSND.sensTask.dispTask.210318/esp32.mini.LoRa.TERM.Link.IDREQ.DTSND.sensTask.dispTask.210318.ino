...
while(true)
  ...
  display.drawString(0, 0, "Term - mode (1)");
...
Serial.begin(9600);
  ...
  dqueue = xQueueCreate( queueSize, 24);  // 24 characters
  ...
  Serial.println("sensor_Task created...");
...
void onReceive(int packetSize) 
{
  if(packetSize==32)
    ...
    if(rcf.pack.con[0]==0x12 && rcf.pack.did==termID)  // received IDACK packet 
      ...
      Serial.println("Received IDACK"); xQueueReset(dqueue);  // reset queue to store only the last values
      xQueueSend(dqueue, "Received IDACK", portMAX_DELAY);
      }
    if(rcf.pack.con[0]==0x14 && rcf.pack.did==termID)  // received DTACK packet
      {
      dtsnd_ack=1; gwID=rcf.pack.sid; timeout=rcf.pack.tout;
      Serial.println("Received DTACK");xQueueReset(dqueue);  // reset queue to stor only the last values
      xQueueSend(dqueue, "Received DTACK", portMAX_DELAY);
      } 
