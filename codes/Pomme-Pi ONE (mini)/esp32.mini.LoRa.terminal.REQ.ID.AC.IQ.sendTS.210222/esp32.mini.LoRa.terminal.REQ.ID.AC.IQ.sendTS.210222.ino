...
void loop() 
{
  if (runEvery(3000)) 
    ...
    if(data_ack==0)
      ...
      Serial.println();Serial.println("DATA sent");
      }
    }
...
void onReceive(int packetSize) 
{
  if(packetSize==32)
    ...
    if(rcf.pack.con[0]==0x04 && rcf.pack.did==nodeID)  // received DATACK frame for this node  
      {
      data_ack=1; recvID=rcf.pack.sid; 
      } 
    Serial.print("This node ID ");Serial.printf("%08X\n",(uint32_t)nodeID);
    Serial.print("GW node ID ");Serial.printf("%08X\n",(uint32_t)recvID);
    Serial.print("Received ACK ");
    }
  Serial.print("Node Receive: ");Serial.println(rcf.pack.con[0],HEX);
}
