...
void send_IDREQ(uint32_t termID,uint8_t serv, char *crypt)
{
conframe_t scf,sccf;
...
void send_IDACK(uint32_t termID,uint32_t gwID,uint8_t serv,uint16_t tout)         
{
conframe_t scf;
  LoRa_txMode();                        // set tx mode
  LoRa.beginPacket();                   // start packet
...
void send_DTACK(uint32_t termID,uint32_t gwID,uint8_t mask,int chan,char *wkey,uint16_t tout)  // TS -send (1)
{
TSframe_t sdf; 
  LoRa_txMode();                        // set tx mode
  LoRa.beginPacket();                   // start packet
  ...
  sdf.pack.con[0]=0x14;sdf.pack.con[1]=mask;  // con[0] depends on the type af ACK frame: IDREQ,DTACK,..
...
void send_DTRCV(uint32_t termID,uint32_t gwID,uint8_t mask,int chan,char *rkey,float stab[],uint16_t tout)  // TS -receive (2)
{
TSframe_t sdf; 
  LoRa_txMode();                        // set tx mode
  LoRa.beginPacket();                   // start packet
...
void send_DTSUBRCV(uint32_t gwID,uint32_t termID,char *topic,char *mess)  // send DTSUB RCVed data
{
MQTTframe_t sdf; 
  LoRa_txMode();                        // set tx mode
  LoRa.beginPacket();
